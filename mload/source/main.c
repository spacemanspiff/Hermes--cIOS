...
void find_and_patch_es(void)
{
	...
	while (((u32) addr) < 0x13A76F00) {
		if(addr[0] == 0x2007) {
			if (addr[1]==0x23A2 || addr[1]==0x4B0B) {
				addr[0]=0x2000;
				direct_os_sync_after_write((void *) addr, 2);
				break;
			}
...
int swi_handler(u32 arg0, u32 arg1,u32 arg2, u32 arg3)
{
	...
	// 32 bits aligned
	if (swi_intr_addr[-4] == 0xdf) 
		swi_intr_addr -= 3;  // 16 bits function
	else 
		swi_intr_addr--; // 32 bits function

	if (*(swi_intr_addr) == 0xcc) {

		switch (arg0) {
			
		case 0: // add SWI handler
			swi_table[arg1]= (void *) arg2;
			break;

			
		case 1: // get EHCI DATA
			return (int) ehci;
			
		case 2: {// memcpy ( RAM cached to cached)
			u32 temp;
			temp = read_access_perm();
			write_access_perm(0xffffffff);
		...
		case 7: // function to call in os_software_IRQ(9)
			system_mode_func = (void *) arg1;
			break;

		case 8: // log buffer function
			switch (arg1) {
			case 1:
		...
		case 9: { // memcpy ( RAM uncached to cached)
			u32 temp;
			temp=read_access_perm();
			write_access_perm(0xffffffff);
		...
		case 17: // get syscalls base
			return ((int) syscall_base);

			
		case 18: // get IOS base
			return ((int) IOS_BASE);

			
		case 19: // get mload version
		...
		if (swi_table[*(swi_intr_addr)]) 
			return swi_table[*(swi_intr_addr)](arg0, arg1, arg2, arg3);
		else 
			return arg0;
	}
	return 0;
...
int main(void)
{
	...
	while(1) {	
...
#else
			swi_mload_memcpy_from_uncached((void *) message->read.data, 
						       (void*)offset, 
						       message->read.length);
		...
		case IOS_WRITE: {
			...
			// Invalidate cache
#if 0
			os_sync_before_read( message->write.data, 
					     message->write.length );
			memcpy((void*)offset, message->write.data, 
			                      message->write.length);
#else
			swi_mload_memcpy((void*)offset, 
					 message->write.data, 
					 message->write.length);
		...
		case IOS_IOCTLV: {
			...
			case MLOAD_RUN_MODULE:
				result = os_thread_create( data_elf.start, NULL, 
							   data_elf.stack, 
							   data_elf.size_stack, 
							   data_elf.prio, 0);

				if (result >= 0)
					os_thread_continue(result);
				break;	

			case MLOAD_RUN_THREAD:

				result = os_thread_create((void *) ioctlv_u32(vec[0]), NULL, 
							  (void *) ioctlv_u32(vec[1]), 
							  ioctlv_u32(vec[2]), 
							  ioctlv_u32(vec[3]), 0);
			...
			case MLOAD_MEMSET:
				result = 0;
				os_sync_before_read((void *)ioctlv_u32(vec[0]), 
						    ioctlv_u32(vec[2]));
				memset((void *) ioctlv_u32(vec[0]), 
				       ioctlv_u32(vec[1]), 
				       ioctlv_u32(vec[2]));
				break;
			...
			case MLOAD_SET_ES_IOCTLV: // changes the current vector for dev/es ioctl (put 0 to disable it)
				result = 0;
				ES_ioctlv_vect i ctlv_u32(vec[0]);
...
int load_elf(u32 elf)
{
	...
	for (n = 0; n < head->phnum; n++) {
		...
		if (entries->type == 4) {
			...
			memcpy((void *) entries->vaddr, 
			       (void *) (elf + entries->offset), 
			       entries->filesz);
